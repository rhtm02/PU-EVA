...
if __name__=="__main__":
    ...
    parser.add_argument('--data_path',
                        default='../dataset/train/PUGAN_poisson_256_poisson_1024.h5')
    ...
    dataset = torch.utils.data.DataLoader(dataset=loader, batch_size=64, shuffle=True,
    ...
    for idx,(inputs,gt,radius) in enumerate(dataset):
        ...
        gt_np = gt.numpy()
        radius_np = radius.numpy()
        np.save('./inputs_point.npy',inputs_np)
        np.save( './gt_points.npy',gt_np)
        np.save( './radius.npy',radius_np)
        break
    print(time.time() - s)
