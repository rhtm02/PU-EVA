...
def fps_and_grouping(points,n_fps,k,use_cuda):
    N, _ = points.shape

    xyz = torch.Tensor(points)
    if use_cuda:
        xyz.cuda()
    fps_indices = fps(xyz, ratio=n_fps / N, random_start=False)

    new_xyz = xyz[fps_indices]

    new_xyz = new_xyz.unsqueeze(0).cuda()
    xyz = xyz.unsqueeze(0).cuda()

    knn = KNN(k=k, transpose_mode=True)
    _, idx = knn(xyz, new_xyz)

    grouped_xyz = index_points(xyz, idx).squeeze()

    # print(grouped_xyz.shape)
    grouped_centers = grouped_xyz.mean(-2).unsqueeze(1)
    # print(grouped_centers.shape, grouped_xyz.shape)

    translation_xyz = grouped_xyz - grouped_centers
    # print(translation_xyz.shape)
    dist = torch.sqrt(translation_xyz[:, :, 0] ** 2 + translation_xyz[:, :, 1] ** 2 + translation_xyz[:, :, 2] ** 2).unsqueeze(-1).max(1)[0].unsqueeze(1)
    # print(dist.shape)

    normalzize_xyz = translation_xyz / dist
    # print(dist.shape, idx.shape)
    # print(grouped_xyz.shape)
    # print(grouped_xyz.mean(-2).shape)

    return normalzize_xyz,dist,grouped_centers
...
def normal_sampling(ratio=4,mean=0,std=0.01,batch_size=32,points=1024,cuda=True):
    if cuda:
        return torch.normal(mean,std,size=(batch_size,points,ratio*3)).cuda()
    else:
        return torch.normal(mean, std, size=(batch_size,points,ratio*3))
if __name__ == '__main__':
    ...
    translation_xyz = grouped_xyz - grouped_centers
    print(translation_xyz.shape)
    dist = torch.sqrt(translation_xyz[:,:,0]**2 + translation_xyz[:,:,1]**2 + translation_xyz[:,:,2]**2).unsqueeze(-1).max(1)[0]
    print(f'dist {dist.shape}')

    normalzize_xyz = translation_xyz / dist.unsqueeze(1)
    # torch.sqrt(normalzize_xyz[:,:,0]**2 + normalzize_xyz[:,:,1]**2 + normalzize_xyz[:,:,2]**2).max(-1)[0]
    print(normalzize_xyz.shape,dist.shape,grouped_centers.shape)

    #print((grouped_xyz - (normalzize_xyz * dist.unsqueeze(1) + grouped_centers)).sum())
